#ifndef BASIC_H
#define BASIC_H

#include <string>
#include <map>
#include <unordered_set>
#include <set>
#include <list>
#include <iostream> 
#include <sstream> 
#include <memory> 
#include <cassert>
#include <ctime>

using std::string;
using std::cout;
using std::endl;

typedef unsigned char Byte;
typedef std::basic_string<Byte> ByteString;

//template<typename T>
//string cnvToHexStr(T v)
//{
//	std::ostringstream stream;
//	stream << std::hex << v;
//	return stream.str();
//}

template<typename T>
string cnvToStr(T v)
{
	std::ostringstream stream;
	stream << v;
	return stream.str();
}

extern string cnvToHexStr(Byte b);
extern string cnvToHexStr(ByteString s);
extern string cnvToHexStr(string s);

extern string cnvToBinStr(string s);

extern string cnvToAsciiStr(ByteString s);
extern ByteString cnvFromAsciiStr(string s);

class Value;

class ValueType
{
private:
	typedef uint8_t Code;
	Code code;

public:
	ValueType() : code(UNINITIALIZED) {}
	ValueType(Code code) : code(code) {}

	operator Code() const { return code; }
	string toStr() const;
	static bool fromStr(string typeStr, ValueType& type);

	static const Code UNINITIALIZED = 0;
	static const Code UNDEFINED = 1;
	static const Code VOID = 2;
	static const Code STRING = 3;
	static const Code BOOLEAN = 4;
	static const Code NUMBER = 5;
};

class Value
{
private:
	ValueType type;
	double number;
	bool boolean;
	string str;

	Value(ValueType type) : type(type), number(0.0), boolean(false) {}
	Value(ValueType type, bool boolean) : type(type), number(0.0), boolean(boolean) {}
	Value(ValueType type, double number) : type(type), number(number), boolean(false) {}
	Value(ValueType type, string str) : type(type), number(0.0), boolean(false), str(str) {}

public:
	Value() : Value(ValueType::UNINITIALIZED) {}

	static Value newUndefined() { return Value(ValueType::UNDEFINED); }
	static Value newVoid() { return Value(ValueType::VOID); }
	static Value newString(string str) { return Value(ValueType::STRING, str); }
	static Value newBoolean(bool boolean) { return Value(ValueType::BOOLEAN, boolean); }
	static Value newNumber(double number) { return Value(ValueType::NUMBER, number); }

	ValueType getType() const { return type; }

	bool isUninitialized() const { return type == ValueType::UNINITIALIZED; }
	bool isUndefined() const { return type == ValueType::UNDEFINED; }
	bool isVoid() const { return type == ValueType::VOID; }
	bool isString() const { return type == ValueType::STRING; }
	bool isBoolean() const { return type == ValueType::BOOLEAN; }
	bool isNumber() const { return type == ValueType::NUMBER; }

	double getNumber() const { assert(isNumber()); return number; }
	bool getBoolean() const { assert(isBoolean()); return boolean; }
	const string& getString() const { assert(isString()); return str; }

	string toStr() const;

	bool operator==(const Value& x) const;
};

class ValueTypes: public std::set<ValueType>
{
public:
	string toStr() const;
};

class Item
{
private:
	// Id of item for unique identification purpose.
	string id;

	// Types of values which can be assigned to the item or which are generated by the item.
	ValueTypes types;

	// Id of link who manages the item. That is, the link over which READ_REQ and WRITE_REQ for the item
	// are sent and on which STATE_IND for the item are received.
	string ownerId;

	// Indicates whether the item can be queried by means of READ_REQ to the owner link.
	bool readable;

	// Indicates whether the item can be modified by means of WRITE_REQ to the owner link.
	bool writable;

	// Indicates whether the owner link automatically reacts with STATE_IND in case WRITE_REQ modifies the item.
	bool responsive;

	// Frequency in which READ_REQ events are generated automatically and passed to the owner link.
	int pollingInterval;

	// Shall unsolicited STATE_IND events be generated in case the owner link has not provided any for
	// a time span?
	bool sendOnTimer;

	// In case sendOnTimer = true:
	// Defines the time span starting from the last STATE_IND until an unsolicited STATE_IND is generated.
	int duration;

	// Shall STATE_IND events generated by the owner link only be forwarded in case the new item values
	// are different from the old ones?
	bool sendOnChange;

	// In case sendOnChange = true:
	// (current value) * (100 - (variation percentage)) <= new value <= (current value) * (100 + (variation percentage)) => ignore event
	float relVariation;

	// In case sendOnChange = true:
	// New item values are suppressed in case they are inside the interval defined by this delta value.
	// (current value) - (variation value) <= new value <= (current value) + (variation value) => ignore event
	float absVariation;

	// In case sendOnChange = true:
	// New item values are suppressed if they are greater than or equal to this one.
	float minimum;

	// In case sendOnChange = true:
	// New item values are suppressed if they are lower than or equal to this one.
	float maximum;

	// Value of last STATE_IND event that was generated by the owner link for the item.
	Value lastSendValue;

	// Time when last STATE_IND event was generated (by the link or internally) and published for the item.
	std::time_t lastSendTime;

	// Time when last READ_REQ event was generated (internally) for the item.
	std::time_t lastPollingTime;

public:	
	Item(string id, ValueTypes types, string ownerId) :
		id(id), types(types), ownerId(ownerId),
		readable(true), writable(true), responsive(true),
		pollingInterval(0), sendOnTimer(false), duration(0),
		sendOnChange(false), relVariation(0.0), absVariation(0.0), minimum(0.0), maximum(0.0),
		lastSendTime(0), lastPollingTime(0) {}

	string getId() const { return id; }
	string getOwnerId() const { return ownerId; }

	const ValueTypes& getTypes() const { return types; }
	bool hasType(ValueType type) const { return types.count(type); }

	void setReadable(bool _readable) { readable = _readable; }
	bool isReadable() const { return readable; }

	void setWritable(bool _writable) { writable = _writable; }
	bool isWritable() const { return writable; }

	void setResponsive(bool _responsive) { responsive = _responsive; }
	bool isResponsive() const { return responsive; }

	void setLastSendValue(const Value& _value) { lastSendValue = _value; }
	const Value& getLastSendValue() const { return lastSendValue; }
	void setLastSendTime(std::time_t _lastSendTime) { lastSendTime = _lastSendTime; }
	std::time_t getLastSendTime() const { return lastSendTime; }

	void setSendOnTimer(bool _sendOnTimer) { sendOnTimer = _sendOnTimer; }
	void setDuration(int _duration) { duration = _duration; }
	bool isSendOnTimerRequired(std::time_t now) const;

	void setSendOnChange(bool _sendOnChange) { sendOnChange = _sendOnChange; }
	void setRelVariation(float _relVariation) { relVariation = _relVariation; }
	void setAbsVariation(float _absVariation) { absVariation = _absVariation; }
	void setMinimum(float _minimum) { minimum = _minimum; }
	void setMaximum(float _maximum) { maximum = _maximum; }
	bool isSendOnChangeEnabled() const { return sendOnChange; }
	bool isSendOnChangeRequired(const Value& value) const;

	void setPollingInterval(int _pollingInterval) { pollingInterval = _pollingInterval; }
	bool isPollingEnabled() const { return pollingInterval > 0; }
	bool isPollingRequired(std::time_t now) const;
	void initPolling(std::time_t now);
	void pollingDone(std::time_t now);

	void validateReadable(bool _readable);
	void validateWritable(bool _writable);
	void validateResponsive(bool _responsive);
	void validatePollingEnabled(bool _enabled);
	void validateType(ValueType _type);
	void validateTypeNot(ValueType _type);
	void validateOwnerId(string _ownerId);
};

class Items: public std::map<string, Item>
{
public:
	void add(Item item) { insert(value_type(item.getId(), item)); }
	bool exists(string itemId) const { return find(itemId) != end(); }
	const Item& get(string itemId) const { auto pos = find(itemId); assert(pos != end()); return pos->second; }
	string getOwnerId(string itemId) const;

	Item& validate(string itemId);
};

class EventType
{
private:
	typedef uint8_t Code;
	Code code;

public:
	EventType() : code(STATE_IND) {}
	EventType(Code _code) : code(_code) {}

	operator Code() const { return code; }
	string toStr() const;
	static bool fromStr(string typeStr, EventType& type);

	static const Code STATE_IND = 0;
	static const Code WRITE_REQ = 1;
	static const Code READ_REQ = 2;
};

class Event
{
private:
	// Id of link which generated the event.
	string originId;
	
	// Id of item for which the event occurs. 
	string itemId;
	
	// STATE_IND, WRITE_REQ or READ_REQ.
	EventType type;
	
	// In case of STATE_IND the current value of the item. For WRITE_REQ it is the new value which should
	// be assigned to the item. READ_REQ events do not make use of it. 
	Value value;
	
public:
	Event(string _originId, string _itemId, EventType _type, const Value& _value) : 
		originId(_originId), itemId(_itemId), type(_type), value(_value) {}
	string getOriginId() const { return originId; }
	string getItemId() const { return itemId; }
	EventType getType() const { return type; }
	const Value& getValue() const { return value; }
	void setValue(const Value& _value) { value = _value; }
};

class Events: public std::list<Event>
{
public:
	void add(Event event) { push_back(event); }
	void add(Events& events) { splice(begin(), events); }
};

#endif
